...
void draw_map(Game_State *state, int width, int height) {
  for (int i = 0; i < state->n_lives; i++) {
    if (state->lives[i].isEnable) {
      driver_print_element(state->lives[i].position.x,
                           state->lives[i].position.y + STATUS_BOARD_OFFSET,
                           'V');
    }
  }
  ...
  for (int i = 0; i < state->n_monsters; i++) {
    if (state->monsters[i].isEnable) {
      driver_print_monster(state->monsters[i].position.x,
                           state->monsters[i].position.y + STATUS_BOARD_OFFSET,
                           state->monsters[i].orientation);
    }
  }

  for (int i = 0; i < state->n_walls; i++) {
    driver_print_element(state->walls[i].x,
                         state->walls[i].y + STATUS_BOARD_OFFSET, 'P');
  }

  for (int i = 0; i < state->n_weapons; i++) {
    if (state->weapons[i].isEnable) {
      driver_print_element(state->weapons[i].position.x,
                           state->weapons[i].position.y + STATUS_BOARD_OFFSET,
                           'E');
    }
  }
}
...
void draw_dashboard(Player player, game_status_t status) {
  driver_print_statusboard(player.lives, player.score, status.stage_conter,
                           MAP_WIDTH * TILE_SIZE, STATUS_BOARD_OFFSET);
}
//--------------------------------------------

//----- INITIAL MAP FUNCITONS -----//
void fill_wall_positions(Vector2D *position, char map[][MAP_WIDTH], int width,
                         int height, char target_char) {
  int index = 0;
...
void fill_elements_init_data(Element *elements, char map[][MAP_WIDTH],
                             int width, int height, char target_char) {
  int index = 0;
...
void get_initial_position_of_all_elements(Game_State *map,
                                          char map_char[][MAP_WIDTH], int width,
                                          int height) {
  map->n_lives = 0;
  map->n_walls = 0;
  map->n_weapons = 0;
  ...
  fill_elements_init_data(map->weapons, map_char, width, height,
                          MAP_WEAPON_SPACE);
  fill_monster_init_data(map->monsters, map_char, width, height,
                         MAP_MONSTER_SPACE);
}
...
void handle_player_movement(Player *player, Game_State *map,
                            uint8_t key_pressed) {
  ...
  if (x_component != 0 && y_component != 0) {
    ...
    */
    ...
    handle_player_weapon(
        player, false);  // if player moves in diagonal, can not use the weapon
  ...
  Vector2D new_player_pos = {
      player->position.x +
          x_component * DEFAULT_PLAYER_VELOCITY * get_frame_time(),
      player->position.y +
          y_component * DEFAULT_PLAYER_VELOCITY * get_frame_time()};
...
void handle_player_monster_interation(Player *player, Game_State *map) {
  for (int i = 0; i < map->n_monsters; i++) {
    ...
    if (monster->isEnable &&
        check_monster_player_colision(player->position, monster->position)) {
      handle_player_damage(player, 1);
    }

    if (monster->isEnable &&
        check_monster_weapon_colision(player, monster->position)) {
...
void handle_monster_movement(Game_State *map_data) {
  for (int i = 0; i < map_data->n_monsters; i++) {
    ...
    if (monster->last_change_time <= 0) {
      ...
      monster->last_change_time =
          0.5f + ((float)rand() / RAND_MAX) *
                     1.0f;  // random time to change orientaton
    }
...
int init_game_data(int stage_no, bool keep_player_status, Player *player,
                   Game_State *Map_Data) {
  char arq_path[50] = {0};
  ...
  get_initial_position_of_all_elements(Map_Data, map_matrix, MAP_WIDTH,
                                       MAP_HEIGHT);

  if (DEBUG_PRINTS) {
    printf("\nMonstros: %d, Vidas: %d, Armas: %d, Paredes: %d\n",
           Map_Data->n_monsters, Map_Data->n_lives, Map_Data->n_weapons,
           Map_Data->n_walls);
  }
  return 0;
}
...
// ---- HANDLE STAGES ---- //
int handle_stage_transition(game_status_t *game_status, Player *Link,
                            Game_State *Map_Data) {
  static int stage_transition_state =
      0;  // 0 = esperando vitória, 1 = mostrar vitória, 2 = ranking final
...
int game_loop(bool is_a_new_game) {
  ...
  int transition_result =
      handle_stage_transition(&game_status, &Link, &Map_Data);
